...
// Bundle css and js based on build tags in nunjucks templates
...
gulp.task('useref', 'Bundle CSS and JS based on build tags and copy to `dist/` folder', function () {
  ...
  if (build.isBuild()) {
    ...
    var nunjucksFilesOnly = filter(['**/*.nunjucks'], {restore: true});
    var excludenunjucks = filter(['**','!**/*.nunjucks']);
    ...
    return gulp.src(config.useref.src)
      ...
      .pipe(assets.restore())
      .pipe(useref())
      .pipe(gulpif(config.cacheBust, revReplace({replaceInExtensions: ['.nunjucks', '.css', '.js']})))
      .pipe(nunjucksFilesOnly)
      .pipe(gulp.dest(config.useref.destnunjucks))
      .pipe(nunjucksFilesOnly.restore)
      .pipe(excludenunjucks)
